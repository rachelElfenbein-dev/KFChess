...
#         """
#         print("Piece.on_command called for", self.piece_id, "with cmd", cmd)
...
#         עדכון מצב הכלי בזמן נתון.
#         """
#         print("Piece.update called for", self.piece_id)
...
class Piece:
    def __init__(self, piece_id: str, state_machine):
        """
        אתחול כלי במשחק עם מזהה ומכונת מצבים (state machine).

        :param piece_id: מזהה ייחודי לכלי (למשל "P1", "Knight3")
        :param state_machine: אובייקט StateMachine שמנהל את כל המצבים של הכלי
        """
        self.piece_id = piece_id
    ...
    def on_command(self, cmd: Command, now_ms: int):
        ...
        print("Piece.on_command called for", self.piece_id, "with cmd", cmd)
        if self.is_command_possible(cmd):
            self.state_machine.process_command(cmd, now_ms)

    def reset(self, start_ms: int):
        """
        איפוס הכלי למצב ההתחלתי שלו.
        """
        self.state_machine.current.reset(None)
        self.state_machine.current.update(start_ms)
    ...
    def update(self, now_ms: int):
        ...
        """
        print("Piece.update called for", self.piece_id)
        self.state_machine.update(now_ms)

    def draw_on_board(self, board: Board, now_ms: int):
        """
        ציור הכלי על הלוח
        """
        self.state_machine.current._graphics.get_img().draw_on(board, 4, 7)